...
public class UserController {
    ...
    @PostMapping("/create")
    public ResponseEntity<?> creatUser(@Valid @RequestBody RegisterForm form){
        UserDto result = userService.createUser(form);
        return ResponseEntity.status(HttpStatus.OK).body(result);
    }
    @PutMapping("/{id}")
    public ResponseEntity<?> updateUser(@Valid @RequestBody  UpdateForm form , @PathVariable int id){
